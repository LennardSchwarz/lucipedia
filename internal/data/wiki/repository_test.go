...
func TestCreateOrUpdateRoundTrip(t *testing.T) {
	...
	if stored.Slug != "example" {
		t.Fatalf("expected slug trimmed to 'example', got %q", stored.Slug)
	}
...
func setupRepository(t *testing.T) *Repository {
	...
	gormDB, err := database.Open(database.Options{Path: path})
	if err != nil {
		t.Fatalf("database.Open returned error: %v", err)
	}

	t.Cleanup(func() {
		if closeErr := database.Close(gormDB); closeErr != nil {
	...
	if err := gormDB.WithContext(context.Background()).AutoMigrate(&PageRecord{}); err != nil {
