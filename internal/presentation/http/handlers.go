...
import (
	...
	"lucipedia/app/internal/presentation/http/templates"
)
...
func (s *Server) homeHandler(ctx context.Context, _ *struct{}) (*htmlResponse, error) {
	count, err := s.wiki.CountPages(ctx)
	if err != nil {
...
func (s *Server) allPagesHandler(ctx context.Context, _ *struct{}) (*htmlResponse, error) {
	pages, err := s.wiki.ListPages(ctx)
	if err != nil {
	...
	for _, page := range pages {
		...
		if slug == "" {
			s.recordError(ctx, eris.New("page slug is empty"), "validating page for listing", logrus.Fields{"slug": page.Slug})
			continue
		}
...
func (s *Server) healthHandler(ctx context.Context, _ *struct{}) (*healthResponse, error) {
	...
	if _, err := s.wiki.CountPages(ctx); err != nil {
		s.recordError(ctx, err, "counting pages for health check", nil)
		resp.Body.Status = "degraded"
		resp.Body.Database = "error"
		resp.Status = stdhttp.StatusServiceUnavailable
	}

	if !s.wiki.GeneratorReady() {
		resp.Body.Status = "degraded"
